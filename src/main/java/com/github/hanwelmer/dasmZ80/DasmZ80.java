...
public class DasmZ80 {
  ...
  protected static HashMap<Integer, AssemblyCode> disassemble(String fileName, ByteReader reader,
	...
	try {
	  ...
	  decoded.put(address, new AssemblyCode(0, null));
	  writeOutput(fileName, writer, address, decoded);
  ...
  private static void writeRemainderOfInput(int address, ByteReader reader, AbstractWriter writer) {
	try {
	  ...
	  writer.write(new AssemblyCode(startOfLine, "", ";"));
	  reader.seek(address);
	  Byte nextByte = reader.getByte();
	  String line = String.format(";%04X:", address);
	  line += String.format(" %02X", nextByte);
	  while ((nextByte = reader.getByte()) != null) {
		address++;
		if (address % 16 == 0) {
		  writer.write(new AssemblyCode(startOfLine, "", line));
		  line = String.format(";%04X:", address);
		  startOfLine = address;
		}
		line += String.format(" %02X", nextByte);
	  }
